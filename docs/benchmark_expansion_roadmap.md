...
2. Energy-matched hard negatives show that action magnitude is a real shortcut; after that shortcut is removed, low-dimensional rollout state traces recover 30.4/32 while action-only selectors recover only 8.2-8.4/32, and the state-trace result is unchanged under both manifest-level and fully regenerated rank/candidate-position randomization.
