...
6. Leave-one-task-out EEF+gripper distribution training drops to 25.6/64, so the current result should not be sold as zero-shot cross-task world modeling. It supports few-shot task/contact-conditioned verification: four target-task calibration cases recover 56.8/64 and eight recover 61.0/64.
...
5. Add cross-task adaptation experiments: leave-one-task-out is only 25.6/64, but four-shot and eight-shot calibration recover 56.8/64 and 61.0/64, so the method needs an explicit few-shot task/contact calibration protocol rather than a pure zero-shot claim.
...
8. `--max-train-cases-per-task`: evaluates few-shot target-task calibration. On EEF+gripper distribution features, K=2,4,8 target cases recover 46.8/64, 56.8/64, and 61.0/64, respectively.
