...
6. Leave-one-task-out EEF+gripper distribution training drops to 25.6/64, and full-source source-only adaptation is 0/64, so the current result should not be sold as zero-shot cross-task world modeling. It supports few-shot task/contact-conditioned verification: full-source plus one target case reaches 46.0/64, four reaches 59.2/64, and eight reaches 62.2/64.
...
5. Add cross-task adaptation experiments: leave-one-task-out is only 25.6/64 and full-source source-only adaptation is 0/64, but full-source plus one/four/eight target cases reaches 46.0/64, 59.2/64, and 62.2/64, so the method needs an explicit few-shot task/contact calibration protocol rather than a pure zero-shot claim.
...
8. `--max-train-cases-per-task` and `--train-case-cap-scope`: evaluate few-shot task/contact calibration. On EEF+gripper distribution features, independent K=2,4,8 target cases recover 46.8/64, 56.8/64, and 61.0/64, respectively; balanced shared-onehot K=1 reaches 41.2/64 versus 24.2/64 for independent K=1. Under the stricter full-source plus target-K adaptation protocol, K=0 collapses to 0/64, while K=1,4,8 reach 46.0/64, 59.2/64, and 62.2/64.
