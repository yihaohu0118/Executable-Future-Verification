...
- low-dimensional rollout state traces recover 30.4/32 on the same energy-matched stress test, while adding action stats to state traces is slightly worse at 30.0/32; the state-trace result stays 30.4/32 after both manifest-level randomization and fully regenerated random-position rollouts. Key ablations are more counterintuitive: removing `object-state` improves to 31.4/32, robot-only traces reach 31.2/32, and object low-dimensional keys alone fall to 16/32. Scaling the fully regenerated random-position pool to 64 hard cases makes the effect stronger: action-only endpoint-free stats average 28.4/64, object-only traces stay at 31.0/64, full state reaches 63.0/64, state+action drops to 62.0/64, proprio-only reaches 63.0/64, and robot-only traces reach 64.0/64 across five seeds. This exposes a sharper bottleneck: action adequacy must be conditioned on robot/contact execution feedback rather than action envelope or object-state shortcuts alone.
...
5. Low-dimensional rollout traces recover 63.0/64 on the n16 stress test, while robot-only traces reach 64.0/64 and object-only traces stay at 31.0/64. The signal is not full object-state leakage; it is still a proprio/state proxy rather than a deployable RGB or tactile critic.
...
4. Convert the robot/proprio trace result into a deployable visual/contact/tactile-conditioned selector: the target is to preserve the 63-64/64 proxy gain without simulator-state access.
