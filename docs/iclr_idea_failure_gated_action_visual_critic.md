...
- in the current slice, temporal order is not just less important than expected; shuffled-time action summaries can outperform ordered summaries on Faucet-style failures, while plain bag-of-actions moments do not reproduce the gain.
...
2. Add shuffle-robust or endpoint-dropout calibration for the Faucet gap: no-demo oracle is 7/8, ordered compact action statistics recover only 2/8, bag moments remain near 2/8, and shuffled-time statistics recover 3-5/8 while PickPlace and OpenCabinet reach their 6/8 oracle ceilings.
