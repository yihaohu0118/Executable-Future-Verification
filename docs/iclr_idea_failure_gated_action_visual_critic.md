...
- low-dimensional rollout state traces recover 30.4/32 on the same energy-matched stress test, while adding action stats to state traces is slightly worse at 30.0/32; the state-trace result stays 30.4/32 after both manifest-level randomization and fully regenerated random-position rollouts. Key ablations are more counterintuitive: removing `object-state` improves to 31.4/32, robot-only traces reach 31.2/32, and object low-dimensional keys alone fall to 16/32. This exposes a sharper bottleneck: action adequacy must be conditioned on robot/contact execution feedback rather than action envelope or object-state shortcuts alone.
...
5. Low-dimensional rollout traces recover 30.4/32 on the same stress test and remain 30.4/32 after fully regenerated random-position rollouts. The signal is not full object-state leakage: excluding `object-state` reaches 31.4/32 and robot-only reaches 31.2/32. It is still a proprio/state proxy rather than a deployable RGB or tactile critic.
...
4. Convert the robot/proprio trace result into a deployable visual/contact/tactile-conditioned selector: the target is to preserve the 30-31/32 proxy gain without simulator-state access.
