...
6. Leave-one-task-out EEF+gripper distribution training drops to 25.6/64, so the current result should not be sold as zero-shot cross-task world modeling. It supports few-shot task/contact-conditioned verification: balanced shared-onehot one-shot calibration reaches 41.2/64, and independent four-shot/eight-shot target calibration recovers 56.8/64 and 61.0/64.
...
5. Add cross-task adaptation experiments: leave-one-task-out is only 25.6/64, balanced shared-onehot one-shot calibration reaches 41.2/64, and four-shot/eight-shot target calibration recovers 56.8/64 and 61.0/64, so the method needs an explicit few-shot task/contact calibration protocol rather than a pure zero-shot claim.
...
8. `--max-train-cases-per-task`: evaluates few-shot task/contact calibration. On EEF+gripper distribution features, independent K=2,4,8 target cases recover 46.8/64, 56.8/64, and 61.0/64, respectively; balanced shared-onehot K=1 reaches 41.2/64 versus 24.2/64 for independent K=1.
