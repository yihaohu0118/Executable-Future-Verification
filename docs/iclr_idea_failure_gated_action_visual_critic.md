...
6. Leave-one-task-out EEF+gripper distribution training drops to 25.6/64, so the current result should not be sold as zero-shot cross-task world modeling. It supports task/contact-conditioned verification.
...
5. Add cross-task adaptation experiments: leave-one-task-out is only 25.6/64, so the method needs few-shot task/contact calibration rather than a pure zero-shot claim.
