...
- low-dimensional rollout state traces recover 30.4/32 on the same energy-matched stress test, while adding action stats to state traces is slightly worse at 30.0/32; the state-trace result stays 30.4/32 after both manifest-level randomization and fully regenerated random-position rollouts. Key ablations are more counterintuitive: removing `object-state` improves to 31.4/32, robot-only traces reach 31.2/32, and object low-dimensional keys alone fall to 16/32. Scaling the fully regenerated random-position pool to 64 hard cases makes the effect stronger: action-only endpoint-free stats average 28.4/64, object-only traces stay at 31.0/64, full state reaches 63.0/64, state+action drops to 62.0/64, proprio-only reaches 63.0/64, and robot-only traces reach 64.0/64 across five seeds. A finer robot-key ablation shows that EEF/gripper traces without the broad proprio vector still reach 62.2/64, EEF position+gripper reaches 62.0/64, EEF position alone reaches 60.4/64, and gripper alone reaches only 43.8/64. A summary-statistic ablation makes the mechanism sharper: terminal-only and endpoint summaries stay near 33-43/64, while endpoint-free EEF position distribution statistics reach 62.6/64 and EEF+gripper distribution statistics reach 63.6/64. A non-neural same-task nearest-positive prototype reaches 59/64 on the same EEF+gripper distribution features, but all-task prototypes fall to 21-48/64. Case-heldout independent/shared/task-head MLPs all stay high at 62.4-63.6/64, but leave-one-task-out and no-task-ID source-only transfer stay at 25.6/64. Full-source one-shot adaptation reaches 42.2/64 without task ID and 46.0/64 with shared one-hot; four-shot and eight-shot shared-onehot adaptation reach 59.2/64 and 62.2/64; per-task heads do not beat shared one-hot. This exposes a sharper bottleneck: action adequacy must be conditioned on compact, few-shot task/contact-calibrated robot execution-envelope feedback rather than action envelope, endpoint state, object-state shortcuts, zero-shot cross-task assumptions, or mandatory task-specific heads.
...
4. Energy-matched hard negatives remove that shortcut but current action-only selectors still recover only 28.4/64 against a 64/64 oracle after n16 random-position scaling.
5. Low-dimensional rollout traces recover 63.0/64 on the n16 stress test, while robot-only traces reach 64.0/64 and object-only traces stay at 31.0/64. The signal is not full object-state leakage: EEF/gripper traces without `robot0_proprio-state` still reach 62.2/64, and endpoint-free EEF+gripper distribution statistics reach 63.6/64. A non-neural same-task prototype reaches 59/64, so much of the effect is interpretable; the remaining gap needs task/contact-conditioned learning. It is still a kinematic execution-envelope proxy rather than a deployable RGB or tactile critic.
6. Leave-one-task-out EEF+gripper distribution training and no-task-ID source-only transfer both stay at 25.6/64, so the current result should not be sold as zero-shot cross-task world modeling. It supports few-shot task/contact-conditioned verification: full-source plus one target case reaches 46.0/64 with shared one-hot, four reaches 59.2/64, and eight reaches 62.2/64.
7. Current phenomenon does not prove temporal world modeling; it proves an under-actuation shortcut first, then exposes the need for visual/contact-conditioned discrimination.
8. Need a full low-level policy-generated candidate source or official benchmark demonstrations for external validity.
...
3. Scale the fully regenerated random-position energy-matched pool beyond the current four-task n16 probe.
4. Convert the EEF/gripper execution-envelope result into a deployable visual/contact/tactile-conditioned selector: the target is to preserve the 63-64/64 proxy gain without simulator-state access under task/contact-calibrated evaluation.
5. Add cross-task adaptation experiments: leave-one-task-out and no-task-ID source-only transfer are only 25.6/64, but full-source plus one/four/eight target cases reaches 46.0/64, 59.2/64, and 62.2/64 with shared one-hot, so the method needs an explicit few-shot task/contact calibration protocol rather than a pure zero-shot claim.
...
- `src/umm_reward_evaluator/benchmarks/score_state_trace_prototype_selector.py`
...
4. RoboCasa low-dimensional state-trace selector: tests whether energy-matched failures require rollout state/contact context after action magnitude shortcuts are removed. On the n8 hard-negative pool, action-only endpoint-free stats average 8.2/32 while state traces recover 30.4/32; after fully regenerated random-position rollouts, action-only is 8.4/32 and state traces remain 30.4/32. On the n16 regenerated random-position pool, action-only is 28.4/64, full state is 63.0/64, proprio-only is 63.0/64, and robot-only reaches 64.0/64. Fine-grained key and summary-statistic ablations show that `object-state` is not required and that endpoint-free EEF/gripper execution-envelope statistics are nearly sufficient at 63.6/64.
5. `--state-summary-mode`: separates terminal, endpoint, delta, path, distribution, and no-endpoint state trace summaries. This is the diagnostic that shows endpoint-free execution-envelope statistics carry the hard-negative signal, while terminal and endpoint summaries do not.
6. `score_state_trace_prototype_selector.py`: evaluates non-neural positive-centroid, positive-vs-negative-centroid, and nearest-positive execution-envelope prototypes. Same-task nearest-positive matching reaches 59/64, while all-task prototypes are much weaker, separating interpretable envelope similarity from task/contact conditioning.
7. `--fold-mode task`: evaluates leave-one-task-out generalization. EEF+gripper distribution drops to 25.6/64 without target-task calibration, which prevents overclaiming zero-shot cross-task world modeling.
8. `--max-train-cases-per-task`, `--train-case-cap-scope`, and `--task-mode shared`: evaluate few-shot task/contact calibration. On EEF+gripper distribution features, independent K=2,4,8 target cases recover 46.8/64, 56.8/64, and 61.0/64, respectively; balanced shared-onehot K=1 reaches 41.2/64 versus 24.2/64 for independent K=1. Under the stricter full-source plus target-K adaptation protocol, no-task-ID K=0 is 25.6/64 and K=1 is 42.2/64, while shared-onehot K=1,4,8 reach 46.0/64, 59.2/64, and 62.2/64.
