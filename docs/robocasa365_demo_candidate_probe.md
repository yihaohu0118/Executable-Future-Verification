...
- The max-absolute-action heuristic is a strong baseline on the n16 ordinary no-demo pool. Energy-matched candidates beat that shortcut: on the n16 regenerated random-position hard-negative pool, action-only selectors still leave a large oracle gap at 28.4/64, deterministic heuristics top out at 24/64, and robot/proprio traces close nearly all of it at 64.0/64 and 63.0/64. EEF/gripper traces without `robot0_proprio-state` still reach 62.2/64, so future results should target visual/contact prediction of execution traces, not just more action summaries.
