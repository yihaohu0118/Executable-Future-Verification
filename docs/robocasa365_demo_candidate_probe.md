...
- The max-absolute-action heuristic is a strong baseline on the n16 ordinary no-demo pool. Energy-matched candidates beat that shortcut, action-only selectors still leave a large oracle gap, and n8 robot/proprio traces close most of it at 30-31/32 even under regenerated random-position placement; future results need visual/contact/proprio conditioning, not just more action summaries.
...
4. Scale the fully regenerated random-position hard-negative pool beyond four tasks and add visual/contact-conditioned rollout critics.
