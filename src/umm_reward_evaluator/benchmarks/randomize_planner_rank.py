...
def randomize_case_ranks(case_rows: list[dict[str, Any]], *, seed: int, mode: str) -> list[dict[str, Any]]:
    ...
    chosen_id = original[chosen_idx]["candidate_id"]
    reranked = []
    next_rank = 1
    for row in original:
        payload = dict(row)
        ...
        if payload["candidate_id"] == chosen_id:
            payload["candidate_rank_by_planner"] = 0
        else:
            payload["candidate_rank_by_planner"] = next_rank
            next_rank += 1
...
def main() -> None:
    ...
    parser.add_argument("--mode", choices=["random", "prefer_success", "prefer_failure"], default="random")
    ...
    for case_offset, case_id in enumerate(sorted(grouped)):
        rows.extend(randomize_case_ranks(grouped[case_id], seed=args.seed + case_offset, mode=args.mode))
    ...
    print(json.dumps({"cases": len(grouped), "mode": args.mode, "rank0_success": rank0_success}, sort_keys=True))
